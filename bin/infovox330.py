...
# Ensure this add-on's own folder is importable so the bundled interface
# definitions (_infovox_sapi4) can be found by the 32-bit synth host.
import os as _addon_os
import sys as _addon_sys
_addon_dir = _addon_os.path.dirname(_addon_os.path.abspath(__file__))
...
class SynthDriver(SynthDriver):
	...
	_host = None  # ctypes handle to infovox_host.dll
	...
	def check(cls):
		...
		return (
			os.path.isfile(os.path.join(d, "infovox_host.dll"))
			and os.path.isfile(os.path.join(d, "Ivx330", "Ivx330nt.dll"))
		)

	def _createEngineEnumerator(self):
		"""Load the reg-free host, initialise the patched engine, and return its ITTSEnumW."""
		import os
		import ctypes
		d = os.path.dirname(os.path.abspath(__file__))
		hostPath = os.path.join(d, "infovox_host.dll")
		enginePath = os.path.join(d, "Ivx330", "Ivx330nt.dll")
		voicesDir = os.path.join(d, "Voices Ivx330")
		if SynthDriver._host is None:
			host = ctypes.CDLL(hostPath)
			host.IVX_Init.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p]
			host.IVX_Init.restype = ctypes.c_int
			host.IVX_CreateEnumerator.argtypes = [ctypes.POINTER(ctypes.c_void_p)]
			host.IVX_CreateEnumerator.restype = ctypes.c_int
			SynthDriver._host = host
		host = SynthDriver._host
		rc = host.IVX_Init(enginePath, voicesDir)
		if rc != 0:
			raise RuntimeError("Infovox host init failed (%d)" % rc)
		pv = ctypes.c_void_p()
		hr = host.IVX_CreateEnumerator(ctypes.byref(pv))
		if hr < 0 or not pv.value:
			raise RuntimeError("Infovox CreateEnumerator failed (0x%08X)" % (hr & 0xFFFFFFFF))
		return ctypes.cast(pv, POINTER(ITTSEnumW))
	...
	def __init__(self):
		...
		self._ttsEngines = self._comThread.invoke(self._createEngineEnumerator)
	...
	def _set_voice(self, val):
		...
		if self._ttsCentral:
			...
			self._ttsAudio.terminate()
			self._ttsAudio = None
		# Infovox 330 requires the custom (WASAPI) sink: it queries the sink's
		# format via WaveFormatGet and only sets its native 16 kHz format when the
		# sink reports none, which SynthDriverAudio does. Always use it.
